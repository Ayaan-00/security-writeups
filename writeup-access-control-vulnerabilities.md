...
First step was recon. I checked `/robots.txt`, which disclosed a `Disallow` directive pointing to `/administrator-panel`. Navigating directly to that path loaded the full admin panel without requiring any login or session.

From there I just clicked delete on the target user account. No credentials needed, no session required. Lab solved.
...
The admin panel is at an unpredictable URL that can't be brute forced. But the URL is leaked in the application's front end JavaScript code.
...
The developers attempted security through obscurity by using an unpredictable URL. But they included the URL in client side code that every visitor receives regardless of their role. This is a pattern that shows up frequently in real applications: sensitive endpoints referenced in JavaScript bundles, HTML comments, or API documentation that's served to all users.

During an authorized assessment I conducted on a client's web application, I found a similar issue: default CMS paths were visible in the page source, giving attackers structural information about the application. The principle is the same: anything in client side code is visible to everyone.
...
The application determines whether a user is an admin by reading a cookie called `Admin` that's set to `false` for regular users. Since cookies are stored client side, any user can modify this value.
...
Then I opened the browser's developer tools, went to the Application tab, found the cookies section, and changed `Admin=false` to `Admin=true`. Reloaded the page and the admin panel appeared. Deleted the target user. Lab solved.
...
The application uses GUIDs (Globally Unique Identifiers) for user accounts, which means the IDs can't be guessed or brute forced. But the application leaks user GUIDs in other parts of the app, and the account page doesn't verify that the requesting user is authorized to view the requested account.

### How I Approached It

I logged in with the provided credentials and noted that the account page URL contained my GUID as the `id` parameter. The GUID was long and random, so brute forcing wasn't viable.
...
Using unpredictable identifiers like GUIDs isn't access control. It's obscurity. The application correctly made the IDs unguessable, but it leaked them through the blog post author links, and it never verified that the requesting user had permission to view the requested account.

This is horizontal privilege escalation: a regular user accessing another regular user's data. In a real application, GUIDs might be leaked through API responses, URL parameters in shared links, browser history, referrer headers, or error messages. The defense isn't making IDs harder to guess. The defense is checking authorization on every request.
...
The application's account page pre-fills the user's existing password in a masked input field. While the password appears hidden in the browser, the actual value is present in the HTML source. Combined with broken access control that allows viewing any user's account page, this exposes the administrator's password.
...
This is how real world attacks work. Individual findings combine into attack chains that escalate from low severity to critical. A penetration tester's job isn't just finding individual vulnerabilities but understanding how they interact.
